...
func solve(input []byte) {
	...
	recurse(cleanedInput)
}

func recurse(cleanedInput [][]rune) {
	OldAnswer = Answer
	rows := len(cleanedInput)
	cols := len(cleanedInput[0])

	// Check corners
	if cleanedInput[0][0] == 64 { // Top-left corner
		Answer++
		cleanedInput[0][0] = 46
	}
	if cleanedInput[0][cols-1] == 64 { // Top-right corner
		Answer++
		cleanedInput[0][cols-1] = 46
	}
	if cleanedInput[rows-1][0] == 64 { // Bottom-left corner
		Answer++
		cleanedInput[rows-1][0] = 46
	}
	if cleanedInput[rows-1][cols-1] == 64 { // Bottom-right corner
		Answer++
		cleanedInput[rows-1][cols-1] = 46
	}
